...
WeatherRequester.prototype.parse_json_response = function(json) {
  var label = "In " + json.location.name + ", ";
  var temperature = "the current temperature is " + json.current.temp_c;
...
WeatherRequester.prototype.execute = function(callback) {
  ...
  http.get(this.compute_url(), function(res) {
    ...
    res.on('end', function() {
      ...
      callback(null, self.parse_json_response(json_response));
    })
  }).on('error', function(e) {
